...
//■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
    ...
    if(!location) {
        result.innerHTML = "<span style='color:red;font-weight:bold'>入力が正しく行われていません！</span>";
    }
    ...
    if(number.length < 12) {
        result.innerHTML = "<span style='color:red;font-weight:bold'>桁が足りません（12桁）</span>";
    } 
    ...
    if(debug == 1) {
        ...
        debug_mode.innerHTML = "運転免許番号:<br> 第 " + number + " 号<br>都道府県番号: " + lc_code + "（" + location + "）" + "<br>取得西暦番号: " + yr_code + "（" + year + "）"+ "<br>内部登録番号: " + naibu + "<br>再発行歴回数: " + lost;
    }
})